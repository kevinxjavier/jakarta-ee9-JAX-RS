...
# Change localhost Domain for kevin.cx Domain 
    $ vi $WILDFLY_HOME/standalone/configuration/standalone.xml
        # Replace this Part 1
            <interfaces>         
                <interface name="management">
        ...
        # Replace this Part 2
            <socket-binding-group name="standard-sockets" default-interface="public" port-offset="${jboss.socket.binding.port-offset:0}">
                <!-- // Replace this: -->
                    <socket-binding name="http" port="${jboss.http.port:8081}"/>
                <!-- // With this: -->
                    <socket-binding name="http" port="${jboss.http.port:8081}" interface="public"/>
        ...
        # Replace this Part 3
            <!-- // Replace this: -->
             ...
             <!-- // With this: -->
                <subsystem xmlns="urn:jboss:domain:undertow:14.0" default-virtual-host="kevin.cx" default-servlet-container="default" default-server="default-server" statistics-enabled="${wildfly.undertow.statistics-enabled:${wildfly.statistics-enabled:false}}" default-security-domain="other">
                    <host name="kevin.cx" alias="localhost">

        # Replace this Part 4
            <!-- // Replace this: -->
                <wsdl-host>${jboss.bind.address:127.0.0.1}</wsdl-host>
            <!-- // With this: -->
                <wsdl-host>kevin.cx</wsdl-host>
...
# WILDFLY WEBSERVICE (WADL URL)
```
        # Runtime   >   Server (Choose it)  >   Monitor >   JAX-RS >   Rest Resource >   (MyWebService) >   WADL URL "Show also the methods like openAPI"
```
...
```
    ...
    $ curl http://localhost:8081/jakarta-ee9-jaxws/CourseServiceWSImpl?wsdl
	
    # GET
	$ curl http://localhost:8081/jakarta-ee9-jaxrs/api/courses
	
    # GET /{id}
	$ curl http://localhost:8081/jakarta-ee9-jaxrs/api/courses/1
	
    # POST "xml"
    $ curl --location 'http://localhost:8081/jakarta-ee9-jaxrs/api/courses/' \
--header 'Content-Type: application/xml' \
--data '<course>
        <description>Learning to Develope</description>
        <duration>1</duration>
        <instructor>Martin Fowler</instructor>
        <name>Lenguage C</name>
    </course>'
...
--data '{
    "description": "Learning to Develope",
    "duration": 1,
    ...
    # PUT "xml"
    $ curl --location --request PUT 'http://localhost:8081/jakarta-ee9-jaxrs/api/courses/20' \
--header 'Content-Type: application/xml' \
--data '<course>
    <description>Learning to Develope</description>
    <duration>1</duration>
	...
	# PUT "json"
	$ curl --location --request PUT 'http://kevin.cx:8081/jakarta-ee9-jaxrs/api/courses/1' \
--header 'Content-Type: application/json' \
--data '{
        "description": "Microservices Essentials",
        "duration": 1.0,
        "id": 33,
        "instructor": {
            "id": 2
    ...
    # DELETE /{id}
    $ curl -X DELETE http://localhost:8081/jakarta-ee9-jaxrs/api/courses/22
    $ curl --location --request DELETE 'http://localhost:8081/jakarta-ee9-jaxrs/api/courses/22'
...
# RUN
```
    $ java -jar .\target\jakarta-ee9-jaxrs-client.jar 
```
